...
physx::PxRigidDynamic* physics::PhysicsController::addDynamicActor(std::shared_ptr<elix::GameObject> actor) const
{
    ...
    const auto& transformation = actor->getComponent<elix::components::TransformComponent>();
    const physx::PxTransform transform(physx::PxVec3(transformation->getPosition().x, transformation->getPosition().y, transformation->getPosition().z));
    ...
    physx::PxShape* shape = m_physics->createShape(physx::PxBoxGeometry(transformation->getScale().x * 0.5f, transformation->getScale().y * 0.5f, transformation->getScale().z * 0.5f), *material);
